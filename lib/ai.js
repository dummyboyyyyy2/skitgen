...
/**
 * @param {Object} params
 * @param {"gemini"|"anthropic"} params.provider - which provider to call.
...
export async function callAI({ provider, system, prompt, maxTokens = 1000, options = {} }) {
  const { lite = false, retries, onRetry, temperature } = options;
  ...
  throw new Error(`Unknown provider: "${provider}". Expected "gemini" or "anthropic".`);
}
