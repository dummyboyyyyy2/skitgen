...
const getRetryDelay = (status, attempt) => Math.min(2000 * Math.pow(2, attempt), 16000); // 2s → 4s → 8s → 16s
...
export async function callGemini(system, userContent, maxTokens = 1500, options = {}) {
  const { retries = 4, onRetry = null, model: modelOverride = null, temperature, json = false } = options;
