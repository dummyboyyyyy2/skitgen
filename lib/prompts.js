...
export const buildIdeaPrompt = (formatLabel, formatDesc, dna, avoidNotes = []) => {
  const dnaContext = dna
    ? `\nThis creator's learned Comedy DNA (from analyzing his real scripts):\n${summarizeDNAForPrompt(dna)}\n\nUse these actual instincts — don't just generate generic premises. The ideas should create natural openings for the instincts and modes listed above. Vary which mode/instinct each idea leans on so the 5 ideas don't all feel the same.`
    : "";
...
He's a confident-idiot type — commits fully to being wrong, warm chaotic energy, sometimes dead calm while saying something unhinged. Taglish voice, Zeke Abella-coded. He riffs on personal life stuff, people-watching, and random hypotheticals. Things that genuinely get to him: fake deep people, annoyingly optimistic people, people who can't take a joke, people who overcomplicate simple stuff, confidently wrong people, overly serious people in casual moments, main-character-syndrome people, people who state the obvious like it's deep.
${dnaContext}${avoidBlock}

Give him 5 ideas he could actually make into a ${formatLabel}. Make them specific and textured — not "something about annoying friends" but the actual specific behavior, like "your friend says he's 5 minutes away and every call has a completely different background noise." Make them feel different from each other, not five versions of the same joke.
...
Each idea must be specific, filmable, and rooted in recognizable relationship behavior rather than generic sitcom conflict. Make the 5 ideas meaningfully different.
