...
// `recentPremises` breaks the repetition loop on "refresh ideas": without it,
// every refresh sends the identical prompt (same DNA, same avoid notes, same
// format), so temperature alone has to supply all the variety — and against a
// well-defined DNA profile the model keeps landing on the same handful of
// highest-probability ideas. This surfaces what was already shown so the model
// is explicitly steered away from repeating it, the same way avoidNotes steers
// it away from disliked patterns.
export const buildIdeaPrompt = (formatLabel, formatDesc, dna, avoidNotes = [], recentPremises = [], scenario = "") => {
  ...
  const scenarioBlock = String(scenario || "").trim()
    ? `\n\nFOCUS AREA FOR THIS BATCH: "${String(scenario).trim()}"\nEvery idea should live inside this area/topic — interpret it naturally and broadly (e.g. "relationship" covers dating, exes, texting habits, anniversaries, etc.), not as one narrow literal scenario. Do not ignore it, and do not force the exact same angle on it 5 times.`
    : "";
  ...
  const recentBlock = Array.isArray(recentPremises) && recentPremises.length
    ? `\n\nALREADY SUGGESTED — DO NOT REPEAT (find genuinely different premises, not reworded versions of these):\n${recentPremises.map((p) => `- ${p}`).join("\n")}\nEND ALREADY SUGGESTED`
    : "";
...
${creativeGuidance}${dnaContext}${scenarioBlock}${avoidBlock}${recentBlock}

Give him UP TO 5 ideas he could actually make into a ${formatLabel} — but quality over quota: only include an idea if it's genuinely strong and distinctive. It is better to return 2 great ideas than to pad to 5 with weaker filler just to hit a count. Make them specific and textured — not generic topics. Let the creator's learned behavior and comedic instincts determine what makes each idea distinctive.

Silently check each idea against the actual creator identity supplied above before including it. Drop — do not include — any idea that feels generic, inconsistent with that identity, or was only added to reach 5. Do not force artificial variety at the expense of the creator's dominant patterns.

Return ONLY a JSON array of 1 to 5 objects (fewer is fine, see above) — no markdown, no preamble, no explanation, no reasoning, nothing before or after the array itself.
...
// See Solo's buildIdeaPrompt for why recentPremises exists: without it every
// "refresh ideas" click is an identical prompt, so the model just re-lands on
// its highest-probability situations. Same fix, mirrored here.
export const buildCoupleIdeaPrompt = (formatLabel, formatDesc, dna, avoidNotes = [], recentPremises = [], scenario = "") => {
  const dnaContext = dna
    ...
    : "";
  const scenarioBlock = String(scenario || "").trim()
    ? `\n\nFOCUS AREA FOR THIS BATCH: "${String(scenario).trim()}"\nEvery situation should live inside this area/topic — interpret it naturally and broadly (e.g. "work" covers commuting, coworkers, WFH friction, job stress spilling into the relationship, etc.), not as one narrow literal scenario. Do not ignore it, and do not force the exact same angle on it 5 times.`
    : "";
  const avoidBlock = Array.isArray(avoidNotes) && avoidNotes.length
    ? `\n\nAVOID LIST:\n${avoidNotes.map((n) => `- ${typeof n === "string" ? n : n.note}`).join("\n")}`
    : "";
  const recentBlock = Array.isArray(recentPremises) && recentPremises.length
    ? `\n\nALREADY SUGGESTED — DO NOT REPEAT (find genuinely different situations, not reworded versions of these):\n${recentPremises.map((p) => `- ${p}`).join("\n")}\nEND ALREADY SUGGESTED`
    : "";
  const guidance = dna
  ...
  return `Suggest UP TO 5 distinct couple comedy situations for a ${formatLabel || "couple comedy"}${formatDesc ? ` (${formatDesc})` : ""} — but quality over quota: only include a situation if it's genuinely strong and distinctive. It is better to return 2 great situations than to pad to 5 with weaker filler just to hit a count.${dnaContext}${scenarioBlock}${avoidBlock}${recentBlock}

${guidance} If two ideas share a learned dynamic, that is acceptable when the actual situation is different.

Silently check each idea against the specific couple identity supplied above. Drop — do not include — anything generic, inconsistent with that identity, or only added to reach 5. Do not write out this checking process.
