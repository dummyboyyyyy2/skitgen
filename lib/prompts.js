...
FORMATS — different rooms, not different formulas: SKIT (scene with dialogue, ends whenever it naturally should), ONE-LINER (one thought, no windup), RANT (starts normal, logic spirals, length follows the spiral not a target), POV (viewer is the other person), CHARACTER BIT (commits to a type for the whole thing), ROAST (sharp, earned, never just mean), COMMENTARY (thinking out loud, casual). Trust the format instead of measuring it.
...
Notice: the observation is instantly recognizable, the escalation builds through specific behavior not random chaos, there's a quoted internal voice that sounds like an actual excuse a person would make, and the ending recontextualizes the whole thing in one line without over-explaining it.
`;

// Deliberately separate from COMEDY_PROFILE. The verify/repair pass isn't
// inventing anything — it's judging an already-written script — so it
// doesn't need the Creative Invention or DNA-handling sections (those are
// instructions for how to originate a premise, not how to check one). Giving
// the verifier the full generation profile wastes tokens on every call and
// risks nudging it toward an "inventing" mindset instead of a "checking" one,
// which cuts against the default-to-leaving-it-alone instinct the
// verification prompt itself is built around. Keep only what's needed to
// judge voice fit and catch real violations.
export const COMEDY_VERIFIER = `
You are the quality-control editor reviewing an already-written comedy script for one specific Filipino content creator, before it reaches him. You are not inventing anything — your only job is to check the script against the brief and repair genuine problems, per the instructions in the check itself.

CREATOR VOICE (for judging fit, not for writing from scratch): He's a Confident Idiot — commits fully to wrong/absurd claims, defends rather than retreats when challenged, never mean, always lovable. Voice is modern conversational Taglish, Gen Z Filipino, natural code-switching. Fillers ("eh," "kasi," "ano," "parang," "talaga," "noh," "ha") only where a real person would actually say them. Short bursts, sometimes unfinished. No Bisaya, ever.

Generic AI-comedy phrasing to flag: "well that escalated quickly," "plot twist," "little did they know," "I was today years old," "make it make sense," "bro really thought," or similar manufactured wit that isn't specifically his.
`;

export const FORMATS = [
  { id: "skit", label: "Skit", icon: "🎬", desc: "Scene with dialogue" },
  { id: "oneliner", label: "One-Liner", icon: "⚡", desc: "Quick, straight to camera" },
  { id: "rant", label: "Rant", icon: "🔥", desc: "Starts normal, spirals" },
  { id: "pov", label: "POV", icon: "👁", desc: "Viewer is in the scene" },
  { id: "character", label: "Character Bit", icon: "🎭", desc: "Full character, no breaking" },
  { id: "roast", label: "Roast", icon: "💀", desc: "Sharp, earned, no mercy" },
  { id: "commentary", label: "Commentary", icon: "🎙", desc: "Thinking out loud" },
];

// ─── Relevance scoring, used to trim strongest_comedic_instincts / comedy_modes
// down to whichever ones actually bear on the current topic, instead of always
// dumping the entire learned profile into every prompt. Pure keyword overlap —
// no extra model call, so it costs nothing.
const DNA_STOPWORDS = new Set([
  "the", "a", "an", "and", "or", "but", "of", "in", "on", "at", "to", "for",
  "with", "is", "are", "was", "were", "be", "been", "this", "that", "he",
  "she", "it", "his", "her", "its", "when", "while", "about", "into", "from",
  "as", "if", "so", "just", "really", "very", "get", "gets", "got", "like",
  "because", "your", "you", "there", "their", "them", "than",
]);
...
function tokenizeForRelevance(str) {
  return String(str || "")
    ...
    .replace(/[^a-z0-9\s]/g, " ")
    .split(/\s+/)
    .filter((w) => w.length > 2 && !DNA_STOPWORDS.has(w));
}

function relevanceScore(text, queryTokenSet) {
  if (!text || !queryTokenSet.size) return 0;
  const textTokens = tokenizeForRelevance(text);
  let score = 0;
  for (const t of textTokens) if (queryTokenSet.has(t)) score += 1;
  return score;
}

// Picks the top N items by relevance to `focus`. If focus is empty, or fewer
// than `keep` items exist, or nothing scores above 0 (topic shares no real
// vocabulary with the DNA yet — e.g. a brand-new profile), returns the
// original list untouched rather than guessing.
function trimByRelevance(items, keep, focus, textOf) {
  if (!Array.isArray(items) || items.length <= keep || !focus) return items;
  const queryTokens = new Set(tokenizeForRelevance(focus));
  if (!queryTokens.size) return items;
  const scored = items.map((item, i) => ({ item, i, score: relevanceScore(textOf(item), queryTokens) }));
...
// `focus` is free text describing what THIS generation is actually about
// (topic + context, typically) — when given, the instinct/mode lists are
// trimmed to whichever entries best match it, so the model isn't asked to
// weigh its entire personality against one narrow request. Global traits
// (avoid lists, ending patterns, anti-patterns, etc.) are always kept in
// full since they apply regardless of topic and are usually short anyway.
export function summarizeDNAForPrompt(dna, focus = "") {
  ...
  const instincts = trimByRelevance(
    dna.strongest_comedic_instincts,
    4,
    focus,
    (i) => `${i.instinct} ${i.description}`
  );
  if (instincts?.length) {
    lines.push(
      "Strongest instincts: " +
  ...
  const modes = trimByRelevance(
    dna.comedy_modes,
    3,
    focus,
    (m) => `${m.name} ${m.description} ${(m.when_to_use || []).join(" ")}`
  );
  if (modes?.length) {
    lines.push(
      "Comedy modes: " +
...
// Idea generation has no topic yet to filter DNA by relevance (that's what
// summarizeDNAForPrompt's `focus` param does elsewhere). It surfaces the
...
export const buildIdeaPrompt = (formatLabel, formatDesc, dna, avoidNotes = []) => {
  const dnaContext = dna
    ? `\nThis creator's learned Comedy DNA (from analyzing his real scripts):\n${summarizeDNAForIdeas(dna)}\n\nUse these actual instincts — don't just generate generic premises. Let the strength scores guide how often each shows up: across the 5 ideas, his highest-scored instincts/modes should appear more often than his lower-scored ones, roughly proportional to strength — but don't force every idea to use a different one just to look varied. It's fine, and often better, for several ideas to share his dominant instinct expressed in different situations.\n\nThese instinct/mode descriptions tell you what KIND of behavior to invent — they are not phrases to reuse. Do not echo or closely paraphrase their specific wording in your ideas (e.g. if a description says "street-fight threat language," don't write "violent street ambush" or "tactical assault" — that's the same words, not a fresh joke). If you notice your premise sharing distinctive vocabulary with a description, rewrite it in different, situation-specific words before including it.`
    : "";
...
${dnaContext}${avoidBlock}

Give him 5 ideas he could actually make into a ${formatLabel}. Make them specific and textured — not "something about annoying friends" but the actual specific behavior, like "your friend says he's 5 minutes away and every call has a completely different background noise."

Stay at a believable, everyday scale — something that could actually happen in a normal day (a conversation, a store, a family dinner, a group chat). The absurdity should come from his logic/reaction to something small, not from the premise itself being a wild, high-concept scenario (no smuggling rings, no claiming legal ownership of a company, nothing that requires an elaborate backstory to set up). If two ideas end up feeling similar in premise, that's fine as long as his angle into each is genuinely different — don't force artificial variety by reaching for an implausible scenario.

Silently check each idea against this before including it: does this sound like something he'd actually say or do — not just a "funny concept"; is it at a scale a real person would actually encounter; would a generic comedy AI have easily landed here too? Replace any idea that fails — but do not write out this checking process anywhere in your answer.
...
Each: { "premise": "specific idea", "tone": "a short natural description of the vibe, e.g. 'confidently wrong' or 'calm and done with it'", "why": "one sentence why this fits him${dna ? " and his learned Comedy DNA" : ""}" }`;
};

export const buildTonePrompt = (topic, formatLabel, formatDesc, dna) => {
  const dnaContext = dna
    ? `\n\nHis learned Comedy DNA:\n${summarizeDNAForPrompt(dna, topic)}\n\nPick the vibe based on which of his actual comedy modes/instincts naturally fits this specific topic — don't just pick whatever sounds good in general.`
    : "";

  return `A Filipino content creator (confident-idiot energy, warm chaotic underneath, sometimes dead calm delivering insane statements, Taglish voice) wants to make a "${formatLabel}"${formatDesc ? ` (${formatDesc})` : ""} about: "${topic}"${dnaContext}

What's the natural vibe for this specific piece? Don't pick from a rigid list — describe it the way a friend would if they were giving him advice before he films. Something like "play it totally confident like you know exactly what you're talking about" or "just stay flat and unbothered the whole time" or "let this one actually build into chaos."

Return ONLY a JSON object, no markdown, no preamble.
{ "tone": "short 2-4 word natural tone description", "reason": "one honest sentence on why this fits" }`;
};

export const buildRefinePrompt = (originalScript, feedback, dna, selectedMode) => {
  const dnaContext = dna
    ...
    : "";
  return `Here's a comedy script for a Filipino content creator:

${originalScript}

He wants this change: "${feedback}"${dnaContext}

Make that change. Keep everything else about the script that's already working — don't rewrite parts he didn't ask about. If he says "make the ending more abrupt," just fix the ending. If he says "make it more self-deprecating," find where that actually fits naturally without breaking the rest of it.
...
Only add a final --- LINE TO REMEMBER --- section if the script actually ends on a distinct, quotable line — preserve that if it's already there, or if your edit naturally produces one. Not every bit needs this: a cut, a fizzle, a silence, or a quiet reaction is a valid ending too. Do not invent a closing line just to fill this section — if the ending doesn't work that way, leave the section out entirely.`;
};
...
export const buildScriptPrompt = (formatLabel, formatDesc, topic, context, suggestedTone, dna, avoidNotes = [], voiceClips = []) => {
  const dnaBlock = dna
    ? `\n\nHis learned Comedy DNA (from real script analysis — abstract patterns, not joke material; invent independently from the underlying instinct, never retrieve or reword a specific past joke/premise):\n${summarizeDNAForPrompt(dna, `${topic || ""} ${context || ""}`)}\n\nBefore writing, silently pick whichever comedy mode/instinct from his DNA gives THIS specific topic the best natural comedic opportunity — don't default to his strongest overall instinct if it doesn't fit. Then start your reply with a line exactly like:\nMODE: <the mode name you picked>\n...before the STYLE line.`
    : "";

  const clips = Array.isArray(voiceClips) ? voiceClips.filter(Boolean) : [];
  const trimmedClips = trimByRelevance(clips, 2, `${topic || ""} ${context || ""}`, (c) => c);
...
2. Come up with at least 4 genuinely different premises — different ideas, not different phrasings of the same one. Pull from different sources so they're actually distinct: one from an odd detail most people would skim past, one from character logic (what would he specifically, confidently, wrongly believe here), one from taking something ordinary far more seriously than it deserves, one from an unexpected connection to something unrelated.
3. For each candidate, ask: does this create funny BEHAVIOR he'd commit to, or just a funny line sitting on top of a flat situation? Behavior wins.
4. Pick the one that's most surprising while still being something he'd genuinely think — not the loudest one. If every candidate still feels like generic internet comedy, keep going before settling.
...
// A second-pass quality gate for generated scripts. It is deliberately a
// verify-and-repair pass rather than a full rewrite: good jokes should survive.
export const buildScriptVerificationPrompt = (originalScript, avoidNotes = [], generationBrief = "") => {
  const avoidBlock = Array.isArray(avoidNotes) && avoidNotes.length
    ? `\n\nEXPLICIT AVOID LIST — hard constraints:\n${avoidNotes.map((n) => `- ${typeof n === "string" ? n : n.note}`).join("\n")}`
    : "";

  return `QUALITY CHECK — verify this generated comedy script before it reaches the creator.

Your job is to catch genuine problems, not to make the comedy safer or more predictable. An unusual, sharp, or specific comedic choice is not itself a defect — "weird," "unexpected," or "not something I'd have written" are not valid reasons to change a line. Being surprising is the goal, not a warning sign. Default to leaving the script alone.

ORIGINAL GENERATION BRIEF (use this to verify format, topic, tone, and learned DNA intent):
${generationBrief}
...
1. Violates an explicit Avoid List rule.
2. Contains actual generic AI-comedy phrasing (e.g. "well that escalated quickly," "plot twist," "little did they know") — not just "sounds unconventional."
3. Reuses or closely paraphrases a specific joke, line, or premise from the supplied Comedy DNA / source samples, rather than inventing independently from the pattern.
4. Breaks the character's own internal logic (the escalation doesn't follow from what they actually believe), or contains forbidden language (e.g. Bisaya).
5. Doesn't actually fit the requested format, or isn't filmable as written.

If none of these apply, return the script completely unchanged — including if it's bold, deadpan-flat, or a slow burn. If something on this list is genuinely wrong, repair ONLY that specific part. Preserve the strongest premise, jokes, specificity, voice, and ending exactly as written otherwise. Never smooth, soften, average-out, or make a line "clearer" or more "polished" — that is not this pass's job.

Return ONLY the final script in the exact same output format. No critique, no explanation, no markdown fences.`;
};
...
export const buildCoupleIdeaPrompt = (formatLabel, formatDesc, dna, avoidNotes = []) => {
  const dnaContext = dna
    ? `\n\nThis couple's learned Comedy DNA (abstract instincts, not joke material):\n${summarizeDNAForPrompt(dna, formatLabel || "")}\n\nLet the strength scores guide how often each instinct/mode shows up across the 5 ideas — higher-scored ones more often, roughly proportional to strength. Every instinct/mode listed is genuinely part of this couple's real dynamic, not just the highest-scored ones; don't force each idea onto a different one just to look varied.\n\nThese descriptions tell you what KIND of dynamic to invent — they are not phrases to reuse. Do not echo or closely paraphrase their specific wording in your ideas; if your premise shares distinctive vocabulary with a description, rewrite it in different, situation-specific words before including it.`
    : "";
  const avoidBlock = Array.isArray(avoidNotes) && avoidNotes.length
    ? `\n\nAVOID LIST:\n${avoidNotes.map((n) => `- ${typeof n === "string" ? n : n.note}`).join("\\n")}`
    : "";
  return `Suggest 5 distinct, highly relatable couple comedy situations for a ${formatLabel || "couple comedy"}${formatDesc ? ` (${formatDesc})` : ""}.${dnaContext}${avoidBlock}

Each idea must be specific, filmable, and rooted in recognizable relationship behavior rather than generic sitcom conflict. Keep every idea at a believable, everyday scale — an actual moment from real domestic/relationship life (a chore, a text exchange, a grocery run, getting ready to leave the house), not a contrived sitcom plot device, a big dramatic reveal, or a scenario that needs an elaborate backstory to set up. The comedy should come from how these two specific people react to something small, not from the situation itself being unusual. If two ideas land on a similar dynamic, that's fine as long as the specific moment is genuinely different — don't reach for an implausible situation just to make the 5 look more different from each other.

Silently check each idea against this before including it: does this sound like something this specific couple would actually do or say, not just a "funny concept"; is it a scale of situation a real couple would actually encounter; would a generic couple-comedy AI have easily landed here too? Replace any idea that fails — but do not write out this checking process anywhere in your answer.
...
export const buildCoupleVibePrompt = (situation, vibes) => `A couple wants to make short comedy content about this situation: "${situation}"
...
export const buildCoupleTonePrompt = (situation, formatLabel, formatDesc, creativeDna = "") => `A couple comedy creator is about to make a ${formatLabel || "couple comedy"}${formatDesc ? ` (${formatDesc})` : ""} about this situation: "${situation}"

${creativeDna ? `Their current Audience & Tone note: "${creativeDna}"` : "They haven't set an Audience & Tone note yet."}

Suggest a short, concrete Audience & Tone note for how THIS audience should be talked to — style of voice, not the joke itself. Keep it under 20 words.
...
export const buildCoupleScriptPrompt = ({ formatLabel, formatDesc, situation, vibe, location, personalityContrast, dynamic, intensity, flavor, blueprint, creativeDna, dna, avoidNotes = [] }) => {
  ...
  if (dna) lines.push(`\nCOMEDY DNA (highest priority — this couple's learned instincts):\n${summarizeDNAForPrompt(dna, `${situation || ""} ${formatLabel || ""} ${flavor || ""}`)}\nEND COMEDY DNA`);
...
Return ONLY valid JSON, no markdown, no commentary:
{
  ...
  "script": "Full filmable script. Natural dialogue and minimal essential physical actions in brackets.",
...
${dna ? `\nPreserve the couple's learned Comedy DNA and the established relationship logic. Do not replace the concept with generic couple comedy.` : ""}
...
export const buildCoupleVerificationPrompt = (originalResult, generationBrief, avoidNotes = []) => {
  const avoidBlock = avoidNotes.length ? `\n\nAVOID LIST:\n${avoidNotes.map((n) => `- ${typeof n === "string" ? n : n.note}`).join("\n")}` : "";
  return `QUALITY CHECK — verify this generated couple-comedy concept before it reaches the creator.

ORIGINAL GENERATION BRIEF:
${generationBrief}

DRAFT:
${originalResult}${avoidBlock}

Only repair genuine problems: explicit avoid-list violations, generic AI-comedy phrasing, broken relationship/character logic, format mismatch, unfilmable behavior, or a structure that contradicts the requested brief. If it is already good, return it unchanged. Do not polish, flatten, or make it safer merely because a choice is unusual.

Return ONLY valid JSON in the exact same structure. No critique or explanation.`;
};
...
export const COUPLE_COMEDY_PROFILE = `You are a short-form couple-content writer and comedy director.
...
// Deliberately separate from COUPLE_COMEDY_PROFILE, mirroring why Solo keeps
// COMEDY_VERIFIER apart from COMEDY_PROFILE: the verify/repair pass isn't
// inventing a concept, it's judging an already-written one, so it doesn't
// need the full creative-invention/relationship-inference/comedy-flavor
// sections above — those are instructions for originating a concept, not for
// checking one. Giving the verifier the full profile wastes tokens on every
// call and risks nudging it toward an "inventing" mindset instead of a
// "checking" one, which cuts against the default-to-leaving-it-alone instinct
// the verification prompt itself is built around.
export const COUPLE_COMEDY_VERIFIER = `
You are the quality-control editor reviewing an already-generated couple-comedy concept before it reaches the creator. You are not inventing anything — your only job is to check the concept against the brief and repair genuine problems, per the instructions in the check itself.

WHAT GOOD LOOKS LIKE (for judging fit, not for writing from scratch): character logic creates the comedy — behavior grows out of who these two specific people are, not a joke assembled backward. Relationship dynamics should feel inferred, not forced into a fixed straight-man-plus-chaos-agent formula. Dialogue is natural, clipped, and human — contractions, interruptions, unfinished thoughts. Nobody explains the joke or narrates their own feelings. Escalation follows from what already happened, not from randomness. The ending lands through a final action, realization, reversal, silence, or doubling-down — it does not just stop.

Generic-AI-comedy phrasing to flag: "well that escalated quickly," "plot twist," "little did they know," "I was today years old," "make it make sense," or similar manufactured wit. Also flag generic couple stereotypes ("men are like this / women are like this"), arguing just to argue, and a character explaining why something is funny.

Output must remain the exact same JSON structure with the same keys (title, hook, characters, premise, setup, beats, script, visual_actions, shot_list, estimated_runtime, filming_difficulty, props, ending, caption, hashtags) — never change the schema, only the content of individual fields that genuinely need repair.
`;
