...
export async function callOpenRouter(system, userContent, maxTokens = 1500, options = {}) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    if (response.ok) {
      ...
      // Some providers ignore reasoning.exclude and/or still burn the whole
      // budget on reasoning even at low effort. If content is empty but we
      // got a reasoning trace back, surface that instead of silent blank
      // text — imperfect, but better than the app treating a real failure
      // as a successful empty script.
