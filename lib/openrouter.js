...
export async function callOpenRouter(system, userContent, maxTokens = 1500, options = {}) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    if (response.ok) {
      ...
      // ignore reasoning.exclude and leave content empty regardless — but
      // callers like the script-verification pass rely on an empty result
      // here to trigger their own fallback to the last known-good text
      // (see app/api/generate's `verifiedAI.text || text`). Substituting
      // raw chain-of-thought would make that check pass with garbage
      // instead of correctly falling back.
