...
export async function GET() {
  try {
    const rows = await sql`SELECT id, note, source_script, created_at FROM avoid_notes ORDER BY created_at DESC LIMIT ${MAX_AVOID_NOTES}`;
...
export async function POST(req) {
  try {
    ...
    const id = body.id ? String(body.id) : `avoid_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    const rows = await sql`
      INSERT INTO avoid_notes (id, note, source_script, created_at)
    ...
    `;
    await sql`
      DELETE FROM avoid_notes
      WHERE id NOT IN (SELECT id FROM avoid_notes ORDER BY created_at DESC LIMIT ${MAX_AVOID_NOTES})
    `;
